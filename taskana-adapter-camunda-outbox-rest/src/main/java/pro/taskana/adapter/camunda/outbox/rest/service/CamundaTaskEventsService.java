...
public class CamundaTaskEventsService {
  ...
  private static final String SQL_GET_ALL_EVENTS = "select * from %s.event_store where lock_expire "
  ...
  private static final String SQL_GET_COMPLETE_AND_DELETE_EVENTS =
      "select * from %s.event_store where (type = ? OR type = ?) and (lock_expire < ? or lock_expire is null)"
  ...
  public List<CamundaTaskEvent> getEventsFilteredByRetries(Integer remainingRetries) {
    ...
    try {
      ...
      ids = camundaTaskEventsFilteredByRetries.stream().map(CamundaTaskEvent::getId).collect(Collectors.toList());;
      lockEvents(ids, getLockExpireAt(), connection);
  ...
  public List<CamundaTaskEvent> getAllEvents() {
    ...
    try (Connection connection = getConnection();
      ...
      List<Integer> ids = camundaTaskEvents.stream().map(CamundaTaskEvent::getId).collect(Collectors.toList());;
      lockEvents(ids, getLockExpireAt(), connection);
      connection.commit();
    } catch (Exception e) {
  ...
  private Instant getLockExpireAt() {
    Duration lockDuration = OutboxRestConfiguration.getDurationLockExpire();
  ...
  private void lockEvents(List<Integer> ids, Instant lockExpiresAt, Connection connection) {
    ...
    try {
      ...
      preparedStatement2.setTimestamp(1, Timestamp.from(lockExpiresAt));
